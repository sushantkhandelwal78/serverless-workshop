...
```bash
sam-step-functions$ sam build --use-container
```
...
```bash
sam-step-functions$ sam logs -n StockCheckerFunction --stack-name sam-step-functions --tail
```
...
```bash
sam-step-functions$ pip install pytest pytest-mock --user
sam-step-functions$ python -m pytest tests/ -v
```
